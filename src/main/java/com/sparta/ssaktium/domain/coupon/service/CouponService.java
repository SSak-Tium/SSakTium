...
public class CouponService {
    ...
    public String issueCoupon(long userId) {
        ...
        // 비관적 락을 사용하여 미발급 상태의 첫 번째 쿠폰을 조회
        List<Coupon> coupons = couponRepository.findFirstByCouponStatusForUpdate(CouponStatus.UNISSUED);
        Coupon coupon = coupons.isEmpty() ? null : coupons.get(0);
        // 모든 쿠폰이 소진된 경우
        if (coupon == null) {
